...
public class CreateImageCode {
    ...
    private int codeCount = 4;          //生成字符数量
    private int lineCount = 10;         //干扰线数量
    ...
    public CreateImageCode(int weight, int height, int codeCount, int lineCount) {
        this.weight = weight;
        this.height = height;
    ...
    public BufferedImage getImage() {
        ...
        for (int i = 0; i < codeCount; i++)             //画四个字符即可
        {
            String s = randomChar() + "";      //随机生成字符，因为只有画字符串的方法，没有画字符的方法，所以需要将字符变成字符串再画
            sb.append(s);                      //添加到StringBuilder里面
            float x = i * 1.0F * weight / codeCount;   //定义字符的x坐标
